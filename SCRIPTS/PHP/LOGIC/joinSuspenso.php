<?php
    ...
    -- Referencia
    FROM suspenso
    JOIN participante ON suspenso.id_participante_suspenso = participante.id_participante
    JOIN lista ON participante.id_lista = lista.id_lista
    JOIN denuncia ON suspenso.id_denuncia = denuncia.id_denuncia
    ...
    WHERE suspenso.id_denuncia = {$id_denuncia};";
    ...
    if ($joinQuery && count($joinQuery) > 0){
        ...
        $fim_suspensao = $joinQuery[0]['data_hora_fim'];
        // Denuncia
    ...
    $data_inicio_suspensao = new DateTime($inico_suspensao);
    $data_fim_suspensao = new DateTime($fim_suspensao);
